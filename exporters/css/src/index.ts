...
Pulsar.export(
  async (
    ...
    if (context.themeIds?.length) {
      ...
      const themesToApply = context.themeIds.reduce<TokenTheme[]>(
        (acc, themeId) => {
          ...
          if (!theme) {
            throw new Error(
              `Unable to apply theme ${themeId} which doesn't exist in the system.`
            );
          }
          acc.push(theme);
          return acc;
      ...
      ...(Object.values(TokenType)
        .map((type) =>
          styleOutputFile(
            ...
            tokenGroups,
            context.themeIds ??
              (context.themeId ? [context.themeId] : undefined) ??
              undefined
          )
        )
