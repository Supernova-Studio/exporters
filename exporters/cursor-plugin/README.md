# Cursor Plugin exporter

Turn a Supernova Context into a ready-to-install Cursor plugin.

This exporter packages the setup your team would otherwise wire by hand: a context-scoped Supernova MCP connection, selected context skills, optional feedback capture, MCP usage guidance, and Cursor plugin metadata. Export it to a repository, push it to GitHub, and your team can install the same Supernova-aware Cursor setup.

Design-system content such as tokens, documentation pages, components, assets, and Storybook data is not written into the plugin. That data stays live in Supernova and is retrieved through the context-scoped MCP server, so Cursor gets fresh context without committing snapshots to the repository.

## What it packages

The exported plugin uses the standard Cursor plugin layout:
...
- Optional `.cursor-plugin/marketplace.json` and `README.md` for publishing and installing from a repository.

## Why design-system data is not exported

The plugin is an installable setup, not a snapshot of your design system. Tokens, documentation, components, assets, and Storybook data change over time, so Cursor should retrieve them through the Supernova MCP when needed.
...
- **pluginName:** Cursor plugin identifier. Leave empty to use a slug of the context name.
- **pluginVersion:** Version written to `.cursor-plugin/plugin.json`.
...
- **pluginAuthorName**, **pluginAuthorEmail:** Optional author metadata.
- **pluginAuthorUrl:** Optional author URL used as a homepage fallback when **pluginHomepage** is empty.
...
- **includeMarketplaceManifest:** Generate `.cursor-plugin/marketplace.json`.
