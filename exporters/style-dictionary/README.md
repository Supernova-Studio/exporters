...
- **typographyOutputFormat:** Export typography token values as CSS font shorthand (`shorthand`, default) or as expanded composite values (`expanded`) with `fontFamily`, `fontWeight`, `fontSize`, `lineHeight`, `paragraphSpacing`, `textCase`, `letterSpacing`, `paragraphIndent`, and `textDecoration`.
