...
describe('GeneralHelper', () => {
    describe('indent', () => {
        it('should create indentation with default spaces (2)', () => {
            expect(GeneralHelper_1.GeneralHelper.indent()).toBe('  ');
        });
        it('should create indentation with specified spaces', () => {
            expect(GeneralHelper_1.GeneralHelper.indent(4)).toBe('    ');
        });
        it('should handle zero spaces', () => {
            expect(GeneralHelper_1.GeneralHelper.indent(0)).toBe('');
        });
        it('should handle negative spaces', () => {
            expect(GeneralHelper_1.GeneralHelper.indent(-2)).toBe('');
        });
    });
    describe('addDisclaimer', () => {
        it('should format single-line disclaimer correctly', () => {
            ...
            const expected = '/**\n' +
                ...
                ' */\n\n' +
                'const x = 1;';
            expect(GeneralHelper_1.GeneralHelper.addDisclaimer(disclaimer, content)).toBe(expected);
        });
        ...
        it('should return content only when disclaimer is empty', () => {
            const content = 'const x = 1;';
            expect(GeneralHelper_1.GeneralHelper.addDisclaimer('', content)).toBe(content);
        });
        it('should handle empty content with disclaimer', () => {
            ...
            const expected = '/**\n' +
                ' * This is a disclaimer \n' +
                ' */\n\n';
            expect(GeneralHelper_1.GeneralHelper.addDisclaimer(disclaimer, '')).toBe(expected);
        });
    });
});
